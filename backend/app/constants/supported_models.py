"""
backend/app/constants/supported_models.py

Central source of truth for supported base models in the HDFC LLM Pipeline.
"""
...
def resolve_model_config(model_identifier: str) -> Dict[str, Any]:
    """
    Validates and resolves a model identifier (internal ID or HF model ID)
    into the canonical supported model configuration.
...
def resolve_hf_model_id(model_identifier: str) -> str:
    """Returns the canonical Hugging Face Model ID for a given model identifier."""
    return resolve_model_config(model_identifier)["hf_model_id"]


def resolve_internal_model_id(model_identifier: str) -> str:
    """Returns the internal model ID for a given model identifier."""
