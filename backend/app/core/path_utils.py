...
Self-contained artifact path resolution and validation utilities for the backend control plane.
Pure Python + standard library (Path, json) — zero ML or torch dependencies.
"""
import json
import logging
...
AI_ROOT = PROJECT_ROOT / "ml-service"

# Standard artifact search locations
ARTIFACT_SEARCH_DIRS = [
    PROJECT_ROOT / "ml-service" / "artifacts",
...
def resolve_artifact_path(path_input: Union[str, Path, None]) -> Optional[Path]:
    ...
    variations (backend/ai/artifacts vs ai/artifacts vs ml-service/artifacts).
    """
    if not path_input:
        return None
    ...
    # 3. Check stripped prefix variations
