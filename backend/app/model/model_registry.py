...
class Model_Registry(Base):
    ...
    huggingface_path = Column(String(500), nullable=True)
    commit_hash = Column(String(128), nullable=True)
    model_size = Column(Float, nullable=True)
    training_job_id = Column(Integer, ForeignKey("training_job.id", ondelete="SET NULL"), nullable=True)
    evaluation_id = Column(Integer, ForeignKey("evaluation.evaluation_id", ondelete="SET NULL"), nullable=True)
